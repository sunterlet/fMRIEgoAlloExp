            ...
            if pygame.K_8 in active_keys or keys[pygame.K_8]:  # Move backward
                ...
                if math.hypot(new_x, new_y) <= ARENA_RADIUS:
                    ...
                    if not is_moving_forward_backward or movement_stop_time is not None:
                        ...
                        movement_stop_time = None
                    is_moving_forward_backward = True
            if pygame.K_6 in active_keys or keys[pygame.K_6]:  # Rotate left
                ...
                if rotation_start_angle is None:
                    rotation_start_angle = player_angle
            if pygame.K_9 in active_keys or keys[pygame.K_9]:  # Rotate right
                player_angle += ROTATE_SPEED * dt
                # Reset rotation tracking if starting new rotation
                if not is_rotating or rotation_stop_time is not None:
                    angle_rotated = 0.0
...
def run_experiment():
    ...
    if MODE == 'practice':
        ...
        show_fixation_image(screen, game_surface, offset_x, offset_y, fixation_duration, 
                           "white_on_black", all_continuous_logs, current_trial, trial_info, BACKGROUND_COLOR)
        
        # Add condition_type to the fixation entries that were just logged by show_fixation_image()
        # (fixation_utils.py doesn't handle condition_type, so we add it here)
        if all_continuous_logs:
            # Find the last two entries (fixation_start and fixation_end) and add condition_type
