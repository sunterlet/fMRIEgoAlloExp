function [results, exit_early] = PTSODfuncICONSheb_fMRI_Test(filesList,trials, saveDir, SubID, day, trialType, screenNumber)
% fMRI Test version - includes fixation crosses for fMRI scanning
...
% trialType: 'fMRI_example', 'fMRI_test' for filename
    ...
    for f = 1:size(results,1)
        ...
        if memory
            ...
            % Add 6-second fixation screen for memory trials
            fixationStart = GetSecs();
            while GetSecs() - fixationStart < 6
                ...
                % Horizontal line
                Screen('DrawLine', window, black, crossX - crossSize, crossY, crossX + crossSize, crossY, crossThickness);
                % Vertical line
                Screen('DrawLine', window, black, crossX, crossY - crossSize, crossX, crossY + crossSize, crossThickness);
            ...
            % Now reveal the right half (object side) and avatar after fixation cross
            Screen('DrawTexture', window, mainImgTexture, [], mainImgRect);
        ...
        if nargin >= 4  % Only save if saveDir is provided
            ...
            % Keep existing format for this function
        ...
        % Add 6-second fixation screen after each trial
        fixationStart = GetSecs();
        while GetSecs() - fixationStart < 6
            % Draw white background
            Screen('FillRect', window, white);
