...
def run_practice_game():
    """Run the practice game where the player must reach a score of 15.
       This version uses the same resolution as the experiment."""
    ...
    avatar_angle = 0      # 0° points to the right.
    avatar_speed = 3
    rotation_speed = 3    # Degrees per frame
    avatar_size = 15      # Not used when drawing using fixed sizes
    ...
    def draw_arena():
        pygame.draw.circle(screen, WHITE, arena_center, arena_radius, 2)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            # Added key event handling for Escape and Enter:
...
def draw_player_avatar(player_pos, player_angle, color=(255, 0, 0)):
    """Draw an elongated triangle representing the player.
       The tip indicates the facing direction.
