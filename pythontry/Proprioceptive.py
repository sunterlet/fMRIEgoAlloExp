...
# Maximum allowed center distance = ARENA_RADIUS - TARGET_RADIUS = 1.65 - 0.3 = 1.35 m.
target_positions = [
    (-0.65, -1.09),  # distance ~1.27 m
    (1.0, 0.5),      # distance ~1.12 m
    (-1.0, 0.8),     # distance ~1.28 m
    (0.5, -1.2)      # distance ~1.30 m
]
...
WHITE = (255, 255, 255)
HUD_COLOR = (0, 255, 0)
DEBUG_COLOR = (50, 50, 255)   # For drawing debug items

# Increased Window size to ensure the arena is fully visible.
WIN_WIDTH = 1000
WIN_HEIGHT = 800
...
# For logging movement during a single press:
move_key_pressed = None        # 'up' or 'down'
move_start_pos = None          # (x, y) when key pressed

rotate_key_pressed = None      # 'left' or 'right'
rotate_start_angle = None      # angle when rotation key pressed

# Variable to track if player was inside target area (for sound trigger)
target_was_inside = False

# Data logging: list of (timestamp, x, y, angle)
log_data = []
...
def draw_debug_items():
    """Draws debug visuals: arena border, player marker, and target outlines."""
...
while running:
    ...
    # Update movement: forward/backward based on current player_angle
    if keys[pygame.K_UP]:
        rad = math.radians(player_angle)
    ...
    # Update rotation:
    # Left arrow rotates left (counterclockwise): subtract angle,
    # Right arrow rotates right (clockwise): add angle.
    if keys[pygame.K_LEFT]:
        player_angle -= ROTATE_SPEED * dt
    if keys[pygame.K_RIGHT]:
        player_angle += ROTATE_SPEED * dt

    log_data.append((time.time(), player_pos[0], player_pos[1], player_angle))

    # Border detection: if near the arena's edge, play beep sound.
    ...
    # Target detection: check if player is within any target area.
    ...
    if not target_was_inside and is_inside_target:
        ...
        # Keep only the encountered target.
    ...
    if show_debug:
        ...
        # In non-testing mode, you can show minimal instructions if desired.
        font = pygame.font.SysFont("Arial", 20)
        instruction_text = font.render("Use arrow keys to move and rotate.", True, WHITE)
        screen.blit(instruction_text, (20, 20))
    
    # Draw HUD feedback for movement (thermometer) and rotation (clock)
    if move_key_pressed is not None and move_start_pos is not None:
        moved_distance = math.hypot(player_pos[0] - move_start_pos[0],
                                    player_pos[1] - move_start_pos[1])
...
with open("player_log.csv", "w") as f:
    f.write("timestamp,x,y,angle\n")
