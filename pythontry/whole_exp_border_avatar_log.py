...
# Sounds
# ---------------------------
SOUNDS_DIR = "/Volumes/ramot-1/sunt/Navigation/fMRI/sounds/"
...
results_dir = "/Volumes/ramot-1/sunt/Navigation/fMRI/pythontry/results"
...
def draw_player_avatar(player_pos, player_angle, color=(255, 0, 0)):
    """Draws an elongated triangle representing the player.
       The tip indicates the current facing direction.
       The color can be specified (default red)."""
    p_screen = to_screen_coords(player_pos)
    tip_length = 30  # distance from center to tip (pixels)
    base_length = 20  # how far back the base is from the center
    half_width = 17   # half the width of the base

    rad = math.radians(player_angle)

    # Tip of the triangle (pointing forward).
    tip = (
        p_screen[0] + int(tip_length * math.sin(rad)),
        p_screen[1] - int(tip_length * math.cos(rad))
    )

    # Base center: a point behind the player.
    base_center = (
        p_screen[0] - int(base_length * math.sin(rad)),
        p_screen[1] + int(base_length * math.cos(rad))
    )

    # Perpendicular offsets (to form the base width).
    left = (
        ...
        if phase == "exploration":
            ...
            if keys[pygame.K_DOWN]:
                ...
                if math.hypot(new_x, new_y) <= ARENA_RADIUS:
                    ...
                    annotation_marker_pos[1] = new_y
            draw_arena()
            # Draw the annotation marker as a blue triangle.
            draw_player_avatar(annotation_marker_pos, annotation_marker_angle, color=(0, 0, 255))
            font = pygame.font.SysFont("Arial", 20)
            ...
            # In feedback, still show the annotation marker as blue.
            draw_player_avatar(annotation_marker_pos, annotation_marker_angle, color=(0, 0, 255))
            font = pygame.font.SysFont("Arial", 20)
