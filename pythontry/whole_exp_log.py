...
# Sounds
# ---------------------------
# New sounds directory and constant sound files.
...
discrete_filename = f"{player_initials}_discrete_log.csv"
continuous_filename = f"{player_initials}_continuous_log.csv"
...
# Load target sound (constant for all targets)
try:
...
# ---------------------------
# Helper: generate random target positions (if needed)
# ---------------------------
def generate_random_targets(n):
    ...
    """
    targets = []
    valid_radius = ARENA_RADIUS - TARGET_RADIUS
    attempts = 0
    max_attempts = 1000
...
def run_trial(is_training, target_sound, trial_info):
    ...
    # Always use constant target sound.
    ...
    continuous_log = []  # rows with columns: trial_info, phase, time, x, y
    ...
    while not trial_done:
        ...
        # Log continuous data (exploration and annotation phases).
        ...
        if phase == "exploration":
            ...
            if is_training:
                ...
                end_y = p_screen[1] - int(line_length * math.cos(rad))
                pygame.draw.line(screen, (255, 0, 0), p_screen, (end_x, end_y), 2)
                # In training, show target outlines when "k" key is pressed.
                if keys[pygame.K_k]:
                    for pos in current_target_positions:
                        target_screen = to_screen_coords(pos)
...
def run_experiment():
    global all_discrete_logs, all_continuous_logs, training_trial, test_trial
    ...
    for t in range(1, TEST_TRIALS + 1):
        ...
        test_mode = False
        discrete_log, continuous_log = run_trial(test_mode, None, trial_info)
        discrete_log["trial_info"] = trial_info
        for row in continuous_log:
            row["trial_info"] = trial_info
        all_discrete_logs.append(discrete_log)
