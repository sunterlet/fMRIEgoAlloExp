#!/usr/bin/env python3
"""
Wrapper script for running one_target experiment in fMRI mode (inside magnet).
...
def run_fmri_session(participant_id="TEST", run_number=1, trial_number=1, total_trials=1, screen_number=None):
    """
    Run the one_target experiment in fMRI mode.
    ...
    try:
        ...
        one_target_script = os.path.join(script_dir, "one_target.py")
        ...
        print(f"Running fMRI session for participant: {participant_id}, run: {run_number}")
        if screen_number is not None:
        ...
        print(f"fMRI run {run_number} completed successfully.")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error running fMRI session: {e}")
        return False
