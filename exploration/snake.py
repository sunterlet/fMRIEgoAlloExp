...
# Try to import video recording libraries
try:
    import cv2
    HAS_OPENCV = True
except ImportError:
...
parser.add_argument('--record', action='store_true',
                   help='Record trial as video with audio')
...
if screen_number is not None:
    # Attempt to select the requested display for fullscreen mode
    try:
        ...
        # Hint SDL which display to use (helps on Windows multi-monitor setups)
...
def record_frame():
    ...
    global recorded_frames, recording_start_time
    ...
    recorded_frames.append((frame_time, frame_array))

def record_audio_event(sound_type, event_time=None):
    """Record audio event for video.
    
    Args:
        sound_type: Type of sound ("target" or "beep")
        event_time: Optional timestamp. If None, uses current time.
    """
    ...
    if event_time is None:
        event_time = time.time() - recording_start_time
    else:
        # event_time is already relative to recording_start_time
        pass
    
    # Add a small delay to align sound with visual event (sound plays slightly after visual)
    audio_delay = 0.05  # 50ms delay - sound plays slightly after visual event
    event_time = event_time + audio_delay
    
    # Don't allow negative times
    event_time = max(0.0, event_time)
    audio_events.append((event_time, sound_type))

def create_video_with_audio(output_path, fps=60):
    """Create video file with audio from recorded frames and events."""
    if not recorded_frames:
        print("No frames recorded for video!")
        return None
    
    print(f"Creating video with {len(recorded_frames)} frames and {len(audio_events)} audio events...")
    ...
    try:
        ...
        height, width = recorded_frames[0][1].shape[:2]
        ...
        if HAS_OPENCV:
            ...
            for frame_time, frame_array in recorded_frames:
                out.write(frame_array.astype(np.uint8))
            out.release()
        elif HAS_IMAGEIO:
            frame_arrays = [frame[1].astype(np.uint8) for frame in recorded_frames]
            imageio.mimsave(temp_video, frame_arrays, fps=fps, codec='libx264')
        else:
            print("Error: No video library available (opencv or imageio)")
            return None
        
        # Calculate video duration from last frame time
        video_duration = recorded_frames[-1][0] if recorded_frames else 0
        
        # Trim audio events to match video duration
        trimmed_audio_events = [(t, s) for t, s in audio_events if t <= video_duration]
        
        # Create audio track with exact duration
        create_audio_track(trimmed_audio_events, temp_audio, video_duration)
        ...
        if result.returncode != 0:
            print(f"Warning: ffmpeg failed, creating video without audio: {result.stderr}")
...
def create_audio_track(audio_events, output_path, duration):
    """Create audio track with target and beep sounds."""
    try:
        import soundfile as sf
        import librosa
    except ImportError:
        # Create silent audio as fallback
        create_silent_wav(output_path, duration, 44100)
        return
    
    sample_rate = 44100
    
    # Load sound files
    target_sound_data = None
    beep_sound_data = None
    
    if os.path.exists(TARGET_SOUND_PATH):
        try:
            target_sound_data, _ = librosa.load(TARGET_SOUND_PATH, sr=sample_rate)
        except:
            pass
    
    if os.path.exists(BEEP_SOUND_PATH):
        try:
            beep_sound_data, _ = librosa.load(BEEP_SOUND_PATH, sr=sample_rate)
        except:
            pass
    
    # Create audio track
    total_samples = int(duration * sample_rate) + sample_rate  # Add 1 second buffer
    ...
    for event_time, sound_type in audio_events:
        ...
        if sound_type == "target" and target_sound_data is not None:
            sound_length = len(target_sound_data)
            end_index = min(sample_index + sound_length, total_samples)
            if end_index > sample_index:
                available_length = end_index - sample_index
                if available_length <= len(target_sound_data):
                    sound_stereo = np.column_stack([target_sound_data[:available_length], 
                                                    target_sound_data[:available_length]])
                    audio_track[sample_index:end_index] += sound_stereo
        
        elif sound_type == "beep" and beep_sound_data is not None:
            # Beep loops continuously, so we'll add it for the duration it should play
            # Since beep plays while near border, add it for a reasonable duration (0.5s)
            # or until the next event/end of track
            beep_duration = 0.5  # Play beep for 0.5 seconds
            beep_samples = int(beep_duration * sample_rate)
            end_index = min(sample_index + beep_samples, total_samples)
            if end_index > sample_index:
                available_length = end_index - sample_index
                # Loop the beep sound if needed
                beep_segment = beep_sound_data[:min(available_length, len(beep_sound_data))]
                if available_length > len(beep_sound_data):
                    # Repeat the beep sound to fill the duration
                    repeats = (available_length + len(beep_sound_data) - 1) // len(beep_sound_data)
                    beep_segment = np.tile(beep_sound_data, repeats)[:available_length]
        ...
        while time.time() - start_time < duration:
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    ...
                    sys.exit()
            clock.tick(15)
            # Record frame during duration wait
            record_frame()
    else:
...
def run_practice_game():
    ...
    global recorded_frames, audio_events, recording_start_time
    ...
    if MODE == 'practice':
        ...
        # Show fixation for the determined number of TRs (skip if recording)
        if not RECORD_VIDEO:
            fixation_start_time = time.time()
            ...
            # Skip initial fixation when recording
            print('Skipping initial fixation (recording mode)')
            # Initialize recording start time if not already set
            if RECORD_VIDEO and recording_start_time is None:
                recording_start_time = time.time()
        ...
        if not RECORD_VIDEO:
            ...
            print('Showing instruction for 1 TR (recording mode)...')
            show_image(os.path.join(INSTRUCTIONS_DIR, "2.png"), duration=TR)
    ...
    while running:
        ...
        if math.hypot(player_pos[0], player_pos[1]) >= (ARENA_RADIUS - BORDER_THRESHOLD):
            if beep_sound is not None and beep_channel is not None:
                if not beep_channel.get_busy():
                    # Record audio event at the exact moment of visual event (before sound plays)
                    record_audio_event("beep", current_time)
        ...
        if distance([tip_x, tip_y], target_pos) < target_radius:
            ...
            target_pos = random_position_in_arena()
            # Record audio event at the exact moment of visual event (before sound plays)
            # This ensures synchronization - the sound will play slightly after but we want it aligned
            record_audio_event("target", target_reach_time)
    ...
    if MODE == 'practice':
        ...
        # Show fixation at the end (2 seconds if recording, 4 TRs otherwise)
        if RECORD_VIDEO:
            end_fixation_duration = 2.0  # 2 seconds when recording
            print('Showing end fixation (2 seconds)...')
        else:
            end_fixation_duration = 4 * TR  # 4 TRs normally
            print('Showing end fixation (4 TRs)...')
        
        end_fixation_logs = []
        end_fixation_start_entry = {
            "RealTime": datetime.now().strftime('%H:%M:%S.%f')[:-3],
            "trial_time": game_duration,
            "trial": trial_info,
            "phase": "fixation",
        ...
        end_fixation_logs.append(end_fixation_start_entry)
        
        end_fixation_start_time = time.time()
        # Record the end time for video trimming
        end_fixation_end_time = end_fixation_start_time + end_fixation_duration
        
        while time.time() - end_fixation_start_time < end_fixation_duration:
            screen.fill(BACKGROUND_COLOR)
            game_surface.fill(BACKGROUND_COLOR)
        ...
        end_fixation_end_entry = {
            "RealTime": datetime.now().strftime('%H:%M:%S.%f')[:-3],
            "trial_time": game_duration + end_fixation_duration,
            "trial": trial_info,
            "phase": "fixation",
        ...
        end_fixation_logs.append(end_fixation_end_entry)
        
        # Add end fixation logs to continuous log
    ...
    if RECORD_VIDEO and recorded_frames:
        # Trim frames to end exactly after final fixation (for fMRI mode)
        if MODE == 'fmri' and 'end_fixation_end_time' in locals() and recording_start_time is not None:
            max_frame_time = end_fixation_end_time - recording_start_time
            recorded_frames = [(t, f) for t, f in recorded_frames if t <= max_frame_time]
            if RECORD_VIDEO:
                print(f"Trimmed video to end at {max_frame_time:.2f}s (after 2 seconds of final fixation)")
            else:
                print(f"Trimmed video to end at {max_frame_time:.2f}s (after 4 TRs of final fixation)")
        ...
        if MODE == 'fmri':
            if run_number == 1:
                ...
                run_context = f"run{run_number}"
            video_filename = os.path.join(results_dir, 
                f"{player_initials}_{run_context}_snake{current_trial}_video.mp4")
        else:
            video_filename = os.path.join(results_dir, 
                f"{player_initials}_snake_practice_video.mp4")
        create_video_with_audio(video_filename, fps=60)
