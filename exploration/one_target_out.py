#!/usr/bin/env python3
"""
Wrapper script for running one_target experiment in practice mode (outside magnet).
...
def run_practice_session(participant_id="TEST", screen_number=None):
    """
    Run the one_target experiment in practice mode.
    ...
    try:
        ...
        print(f"Running practice session for participant: {participant_id}")
        if screen_number is not None:
        ...
        print(f"Error running practice session: {e}")
        return False
