#!/usr/bin/env python3
"""
Wrapper script for running snake vection game in practice mode (outside magnet).
...
def run_practice_session(participant_id="TEST", screen_number=None):
    """
    Run the snake vection game in practice mode.
    ...
    try:
        ...
        print(f"Running snake vection practice session for participant: {participant_id}")
        if screen_number is not None:
        ...
        print("Snake vection practice session completed successfully.")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error running snake vection practice session: {e}")
        return False
