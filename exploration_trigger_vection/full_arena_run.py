...
def run_trial(trial_number, trial_type, participant_id, run_number, total_trials, arena_name=None, screen_number=None, multi_arena_number=None, total_multi_arenas=None, snake_trial_number=None, scanning=False, com_port='com4', tr=2.01, vection=False):
    ...
    # Determine which script to run (all scripts in same directory)
    if trial_type == "snake":
        script_name = "snake_vection.py" if vection else "snake.py"
    elif trial_type == "multi_arena":
        script_name = "multi_arena_vection.py" if vection else "multi_arena.py"
    else:
    ...
    if scanning:
        cmd.extend(["--scanning", "--com", com_port, "--tr", str(tr)])
    ...
    # Pass explicit full arena run identifier to ensure unique logging filenames
    cmd.extend(["--fa-run", str(run_number)])
    ...
    try:
        # Run the trial and stream output live
        return_code = run_and_stream(cmd)
        ...
        if return_code != 0:
            print(f"CRITICAL: {trial_type} trial returned error code {return_code}. Aborting run.")
...
def run_full_arena_run(participant_id, run_number, screen_number=None, scanning=False, com_port='com4', tr=2.01, vection=False):
    ...
    trial_results = []
    next_multi_idx = 0
    multi_arena_trial_counter = 0  # Track which multi_arena trial we're on (1st, 2nd, etc.)
    ...
    for trial_number, trial_type in enumerate(trial_sequence, 1):
        ...
        if trial_type == "multi_arena":
            multi_arena_trial_counter += 1
            current_multi_arena_number = multi_arena_trial_counter
        ...
        if result.get("return_code", 0) != 0:
            abort_code = int(result.get("return_code", 1) or 1)
            print(f"\n{'!'*80}")
            print(f"RUN ABORTED")
            print(f"Reason: Trial {trial_number} ({trial_type}) failed with exit code {abort_code}")
            print(f"{'!'*80}\n")
            # Save partial timing log for debugging (best-effort)
            try:
                base_log_filename = f"{participant_id}_full_arena_run_timing_ABORTED.csv"
                ...
                print(f"Warning: Could not save partial timing log: {e}")
            sys.exit(abort_code)
    ...
    try:
        ...
        # 4 TRs final fixation before finish screen
        print(f'Final fixation: 4 TRs')
