...
%       one_target_run('TS263', 1)       % One Target Run on screen 1
    ...
    % Path to the exploration_trigger_vection experiment directory
    exploration_dir = fullfile(current_dir, 'exploration_trigger_vection');
    
    if ~exist(exploration_dir, 'dir')
        error('Exploration experiment directory not found: %s', exploration_dir);
    end

    % Change to the exploration_trigger_vection experiment directory
    cd(exploration_dir);
