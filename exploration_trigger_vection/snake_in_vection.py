#!/usr/bin/env python3
"""
Wrapper script for running snake vection game in fMRI mode (inside magnet).
...
def run_fmri_session(participant_id="TEST", run_number=1, trial_number=1, total_trials=1, screen_number=None):
    """
    Run the snake vection game in fMRI mode.
    ...
    try:
        ...
        snake_script = os.path.join(script_dir, "snake_vection.py")
        ...
        print(f"Running snake vection fMRI session for participant: {participant_id}, run: {run_number}")
        if screen_number is not None:
        ...
        print(f"Snake vection fMRI run {run_number} completed successfully.")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error running snake vection fMRI session: {e}")
        return False
