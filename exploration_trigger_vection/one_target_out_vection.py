#!/usr/bin/env python3
"""
Wrapper script for running one_target vection experiment in practice mode (outside magnet).
...
def run_practice_session(participant_id="TEST", screen_number=None):
    """
    Run the one_target vection experiment in practice mode.
    ...
    try:
        ...
        cmd = [sys.executable, one_target_script, "practice", "--participant", participant_id]
        
        if screen_number is not None:
            cmd.extend(["--screen", str(screen_number)])
        
        print(f"Running one_target vection practice session for participant: {participant_id}")
        if screen_number is not None:
        ...
        print("One target vection practice session completed successfully.")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error running one_target vection practice session: {e}")
        return False
