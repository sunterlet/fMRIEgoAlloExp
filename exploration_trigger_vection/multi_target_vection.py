...
parser.add_argument('--arena', '-a', default=None,
                   help='Arena name to run (default: None; for practice, use --practice-sequence for full 3-trial flow)')
parser.add_argument('--practice-sequence', action='store_true',
                   help='Run full practice sequence: instructions, trial 1 (garden/training), trial 2 (beach), trial 3 (zoo), thank you')
...
practice_sequence = args.practice_sequence
...
# These are set by multi_target_run.py based on the run configuration
...
if MODE == 'fmri':
    run_prefix = f"MT{mt_run_number}V" if mt_run_number is not None else "MTV"
    continuous_filename = os.path.join(results_dir, f"{player_initials}_{run_prefix}_mt{current_trial}_continuous.csv")
    discrete_filename = os.path.join(results_dir, f"{player_initials}_{run_prefix}_mt{current_trial}_discrete.csv")
else:
...
def show_image(image_path, duration=None, continuous_log=None, trial_info=None):
    ...
    if duration is not None:
        ...
        # Wait for 1 or ENTER key press (matching multi_target.py behavior)
        waiting = True
        ...
        if "Done.png" not in image_path:  # Don't clear screen for thank you screen
            screen.fill(FULLSCREEN_BACKGROUND)
            pygame.display.flip()
...
def run_arena(arena_name, targets, arena_num, total_arenas, training=False, hebrew_names=None, trial_number=None):
    """Run a single arena trial. Exploration: 3D first-person. Annotation: 2D top-down.
    training: If True, minimap is always visible. If False, minimap starts hidden (B key toggles)."""
    ...
    player_angle = 0.0
    phase = "exploration"
    # Training trial: minimap always on. Regular trial: minimap off unless --debug or B pressed.
    show_debug_minimap = training or args.debug
    ...
    LOG_INTERVAL = 0.1
    # Trial type for logging (training vs test)
    trial_started = False
    ...
    while running:
        ...
        if experiment_time - (last_log_time - EXPERIMENT_START_TIME) >= LOG_INTERVAL:
            ...
            log_entry = {
                ...
                "visibility": "training" if training else "test",
                "phase": phase,
                "event": "",
                "x": round(x_log, 3),
                "y": round(y_log, 3),
...
def run_multi_arena_experiment():
    ...
    if MODE == 'practice':
        ...
        # Full practice sequence: 3 trials (training, beach, zoo)
        if practice_sequence:
            # Instructions once before trial 1; arenas: garden, zoo, beach; minimap togglable with B in all
            print("Practice sequence: Instructions")
            show_image(os.path.join(INSTRUCTIONS_DIR, "FA-ins.png"), continuous_log=practice_continuous_log, trial_info="practice_instructions")
            
            for trial_num, current_arena in enumerate(["garden", "zoo", "beach"], start=1):
                targets = arenas[current_arena]
                arena_hebrew_names = hebrew_names.get(current_arena, {})
                print(f"Practice sequence: Trial {trial_num} - {current_arena} (minimap togglable with B)")
                draw_arena_intro(current_arena, trial_num, 3, len(targets), hebrew_arena_names, continuous_log=practice_continuous_log)
                run_arena(f"{current_arena}_practice", targets, trial_num, 3, training=False, hebrew_names=arena_hebrew_names)
            
            # Thank you screen
            print("Practice sequence: Thank you")
            show_image(os.path.join(INSTRUCTIONS_DIR, "Done.png"), continuous_log=practice_continuous_log, trial_info="practice_thank_you")
            return
        
        # Legacy: single-arena practice (thank_you or explicit arena)
        if arena_name == 'thank_you':
            print("Showing thank you screen...")
            show_image(os.path.join(INSTRUCTIONS_DIR, "Done.png"), continuous_log=practice_continuous_log, trial_info=f"{arena_name}_practice")
            return
        
        if arena_name not in arenas:
            print(f"Error: Arena '{arena_name}' not found. Use --practice-sequence for full practice flow.")
            return
        
        targets = arenas[arena_name]
        arena_hebrew_names = hebrew_names.get(arena_name, {})
        print(f"Running single practice trial: {arena_name}")
        ...
        run_arena(f"{arena_name}_practice", targets, 1, 1, training=False, hebrew_names=arena_hebrew_names)
        
    elif MODE == 'fmri':
        # fMRI mode: require arena, load targets
        if arena_name is None or arena_name not in arenas:
            print(f"Error: fMRI mode requires --arena. Arena '{arena_name}' not found.")
            return
        targets = arenas[arena_name]
        arena_hebrew_names = hebrew_names.get(arena_name, {})
        ...
        if trigger_time_value is not None:
            trigger_entry = {
                ...
                "trial_type": "MT",  # Add missing field
        ...
        # Show fixation at start only when scanning - 8 TRs for first trial (4 TRs dummy + 4 TRs trial), 4 TRs for subsequent trials
        if scanning:
            if current_trial == 1:
                fixation_trs = 8
            ...
            print('Showing instruction for 1 TR...')
        show_image(os.path.join(INSTRUCTIONS_DIR, "FA-screen.png"), duration=TR, continuous_log=fixation_logs, trial_info=trial_info)
        ...
        discrete_log, continuous_log = run_arena(f"{arena_name}_test_run{run_number}", targets, 1, 1, training=False, hebrew_names=arena_hebrew_names, trial_number=current_trial)
        ...
        if current_trial == total_trials:
            ...
            if scanning:
                # Final trial (scanning): Show 4 TRs fixation followed by thank you screen
            ...
            # Save logs (before thank you screen)
            save_logs(discrete_log, continuous_log, player_initials)
            # Show thank you screen
            print("Showing thank you screen...")
            show_image(os.path.join(INSTRUCTIONS_DIR, "Done.png"), continuous_log=None, trial_info=None)
        else:
