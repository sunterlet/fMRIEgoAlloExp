#!/usr/bin/env python3
"""
Wrapper script for running one_target vection experiment in fMRI mode (inside magnet).
...
def run_fmri_session(participant_id="TEST", run_number=1, trial_number=1, total_trials=1, screen_number=None):
    """
    Run the one_target vection experiment in fMRI mode.
    ...
    try:
        ...
        one_target_script = os.path.join(script_dir, "one_target_vection.py")
        
        # Build command with optional screen parameter
        cmd = [sys.executable, one_target_script, "fmri", "--participant", participant_id, "--run", str(run_number), "--trial", str(trial_number), "--total-trials", str(total_trials)]
        
        if screen_number is not None:
            cmd.extend(["--screen", str(screen_number)])
        
        print(f"Running one_target vection fMRI session for participant: {participant_id}, run: {run_number}")
        if screen_number is not None:
        ...
        print(f"One target vection fMRI run {run_number} completed successfully.")
        return True
        
    except subprocess.CalledProcessError as e:
        print(f"Error running one_target vection fMRI session: {e}")
        return False
